...
<a id="from-software-collections"></a>
###### 从 Software Collections
...
<a id="from-official-repository"></a>
###### 从官方仓库
...
<a id="from-debianubuntu-repository"></a>
###### 从 Debian/Ubuntu 仓库
...
<a id="from-freebsd-repository"></a>
###### 从 FreeBSD 仓库
...
<a id="automatic-installation-on-rheldebianbsd"></a>
##### RHEL/Debian/BSD 自动安装
...
<a id="installation-nginx-on-centos-7"></a>
#### 在 CentOS 7 上安装 Nginx

<a id="pre-installation-tasks"></a>
###### 安装前任务
...
<a id="download-3rd-party-modules"></a>
###### 下载第三方模块
...
<a id="post-installation-tasks"></a>
###### 安装后任务
...
<a id="installation-openresty-on-centos-7"></a>
#### 在 CentOS 7 上安装 OpenResty
...
<a id="installation-tengine-on-ubuntu-1804"></a>
#### 在 Ubuntu 18.04 上安装 Tengine
...
<a id="installation-nginx-on-freebsd-113"></a>
#### 在 FreeBSD 11.3 上安装 Nginx
...
<a id="installation-nginx-on-freebsd-121-from-ports"></a>
#### 在 FreeBSD 12.1 上安装 Nginx（从 ports）
...
<a id="update-freebsd-ports-tree"></a>
###### 更新 FreeBSD ports 树
...
<a id="analyse-log-file-and-enable-all-recorded-statistics"></a>
###### 分析日志文件并启用所有记录的统计信息
...
<a id="analyse-compressed-log-file"></a>
###### 分析压缩的日志文件
...
<a id="analyse-log-file-remotely"></a>
###### 远程分析日志文件
...
<a id="analyse-log-file-and-generate-html-report"></a>
###### 分析日志文件并生成 HTML 报告
...
<a id="analyse-log-file-and-print-requests-with-4xx-and-5xx"></a>
###### 分析日志文件并打印 4xx 和 5xx 的请求
...
<a id="build-openssl-102-chacha-version"></a>
###### 构建 OpenSSL 1.0.2-chacha 版本
...
<a id="send-request-and-show-response-headers"></a>
###### 发送请求并显示响应头
...
<a id="send-request-with-http-method-user-agent-follow-redirects-and-show-response-headers"></a>
###### 发送请求（HTTP方法/User-Agent/跟随重定向）并显示响应头
...
<a id="testing-ssl-connection-debug-mode"></a>
###### 测试 SSL 连接（调试模式）
...
<a id="testing-ssl-connection-with-sni-support"></a>
###### 测试支持 SNI 的 SSL 连接
...
<a id="testing-ssl-connection-with-specific-ssl-version"></a>
###### 测试特定 SSL 版本的连接
...
<a id="testing-ssl-connection-with-specific-cipher"></a>
###### 测试特定密码套件的 SSL 连接
...
<a id="load-testing-with-apachebench-ab"></a>
##### 使用 ApacheBench (ab) 进行负载测试
...
<a id="test-with-keep-alive-header"></a>
###### 使用 Keep-Alive 头部测试
...
<a id="multiple-paths-with-lua"></a>
###### 多个路径（使用 Lua）
...
<a id="random-server-address-to-each-thread-with-lua"></a>
###### 为每个线程随机分配服务器地址（使用 Lua）
...
<a id="multiple-json-requests-with-lua"></a>
###### 多个 JSON 请求（使用 Lua）
...
<a id="analyse-data-pass-to-and-from-the-threads"></a>
###### 分析传入和传出线程的数据
...
<a id="parsing-wrk-result-and-generate-report"></a>
###### 解析 wrk 结果并生成报告
...
<a id="load-testing-with-locust"></a>
##### 使用 locust 进行负载测试
...
<a id="multiple-paths-with-different-user-sessions"></a>
###### 具有不同用户会话的多个路径
...
<a id="tcp-syn-flood-denial-of-service-attack"></a>
###### TCP SYN 洪水拒绝服务攻击
...
<a id="http-denial-of-service-attack"></a>
###### HTTP 拒绝服务攻击
...
<a id="show-information-about-processes"></a>
##### 显示进程信息
...
<a id="check-segmentation-fault-messages"></a>
##### 检查段错误消息
...
<a id="get-the-list-of-configure-arguments"></a>
##### 获取配置参数列表
...
<a id="check-if-the-module-has-been-compiled"></a>
##### 检查模块是否已编译
...
<a id="show-the-most-accessed-ip-addresses"></a>
##### 显示访问最多的 IP 地址
...
<a id="show-the-most-accessed-ip-addresses-ip-and-url"></a>
##### 显示访问最多的 IP 地址（IP 和 URL）
...
<a id="show-the-most-accessed-ip-addresses-method-code-ip-and-url"></a>
##### 显示访问最多的 IP 地址（方法、状态码、IP 和 URL）
...
<a id="show-the-top-5-visitors-ip-addresses"></a>
##### 显示前 5 名访客（IP 地址）
...
<a id="show-the-most-requested-urls"></a>
##### 显示最常请求的 URL
...
<a id="show-the-most-requested-urls-containing-string"></a>
##### 显示包含 'string' 的最常请求 URL
...
<a id="show-the-most-requested-urls-with-http-methods"></a>
##### 显示带有 HTTP 方法的最常请求 URL
...
<a id="show-the-most-accessed-response-codes"></a>
##### 显示最常访问的响应状态码
...
<a id="analyse-web-server-log-and-show-only-2xx-http-codes"></a>
##### 分析 Web 服务器日志并仅显示 2xx HTTP 状态码
...
<a id="analyse-web-server-log-and-show-only-5xx-http-codes"></a>
##### 分析 Web 服务器日志并仅显示 5xx HTTP 状态码
...
<a id="show-requests-which-result-502-and-sort-them-by-number-per-requests-by-url"></a>
##### 显示返回 502 的请求并按 URL 请求次数排序
...
<a id="show-requests-which-result-404-for-php-files-and-sort-them-by-number-per-requests-by-url"></a>
##### 显示 PHP 文件返回 404 的请求并按 URL 请求次数排序
...
<a id="calculating-amount-of-http-response-codes"></a>
##### 计算 HTTP 响应状态码数量
...
<a id="calculating-requests-per-second"></a>
##### 计算每秒请求数
...
<a id="calculating-requests-per-second-with-ip-addresses"></a>
##### 计算每秒请求数（含 IP 地址）
...
<a id="calculating-requests-per-second-with-ip-addresses-and-urls"></a>
##### 计算每秒请求数（含 IP 地址和 URL）
...
<a id="get-entries-within-last-n-hours"></a>
##### 获取最近 n 小时内的条目
...
<a id="get-entries-between-two-timestamps-range-of-dates"></a>
##### 获取两个时间戳之间的条目（日期范围）
...
<a id="get-line-rates-from-web-server-log"></a>
##### 从 Web 服务器日志获取行速率
...
<a id="trace-network-traffic-for-all-processes"></a>
##### 跟踪所有进程的网络流量
...
<a id="list-all-files-accessed-by-a-nginx"></a>
##### 列出 Nginx 访问的所有文件
...
<a id="check-that-the-gzipstatic-module-is-working"></a>
...
<a id="which-worker-processing-current-request"></a>
##### 哪个 worker 正在处理当前请求
...
<a id="capture-only-http-packets"></a>
##### 仅捕获 HTTP 数据包
...
<a id="extract-user-agent-from-the-http-packets"></a>
##### 从 HTTP 数据包中提取 User-Agent

```bash
tcpdump -ei eth0 -nn -A -s1500 -l | grep "User-Agent:"
```

<a id="capture-only-http-get-and-post-packets"></a>
...
<a id="capture-requests-and-filter-by-source-ip-and-destination-port"></a>
##### 捕获请求并按源 IP 和目标端口过滤
...
<a id="capture-http-requestsresponses-in-real-time-filter-by-get-head-and-save-to-a-file"></a>
##### 实时捕获 HTTP 请求/响应，按 GET、HEAD 过滤并保存到文件
...
<a id="check-closewait-connections"></a>
##### 检查 CLOSE_WAIT 连接
...
<a id="dump-configuration-from-a-running-process"></a>
###### 从正在运行的进程导出配置
...
<a id="show-debug-log-in-memory"></a>
###### 显示内存中的调试日志
...
<a id="configuration-snippets"></a>
#### 配置片段

<a id="nginx-server-header-removal"></a>
##### 移除 Nginx 服务器头部
...
<a id="restricting-access-with-basic-authentication"></a>
##### 使用基本认证限制访问
...
<a id="restricting-access-with-client-certificate"></a>
##### 使用客户端证书限制访问
...
<a id="restricting-access-by-geographical-location"></a>
##### 按地理区域限制访问
...
<a id="dynamic-error-pages-with-ssi"></a>
##### 使用 SSI 的动态错误页面
...
<a id="blockingallowing-ip-addresses"></a>
##### 阻止/允许 IP 地址
...
<a id="limiting-the-rate-of-requests-with-burst-mode"></a>
##### 使用 burst 模式限制请求速率
...
<a id="limiting-the-rate-of-requests-with-burst-mode-and-nodelay"></a>
##### 使用 burst 模式和 nodelay 限制请求速率
...
<a id="limiting-the-rate-of-requests-per-ip-with-geo-and-map"></a>
##### 使用 geo 和 map 按 IP 限制请求速率
...
<a id="limiting-the-number-of-connections"></a>
##### 限制连接数
...
<a id="properly-redirect-all-http-requests-to-https"></a>
##### 正确将所有 HTTP 请求重定向到 HTTPS
...
<a id="proxyrewrite-and-keep-the-original-url"></a>
##### 代理/重写并保留原始 URL
...
<a id="proxyrewrite-and-keep-the-part-of-original-url"></a>
##### 代理/重写并保留部分原始 URL
...
<a id="proxyrewrite-without-changing-the-original-url-in-browser"></a>
##### 代理/重写而不更改原始 URL（浏览器中）
...
<a id="adding-and-removing-the-www-prefix"></a>
...
<a id="modify-301302-response-body"></a>
##### 修改 301/302 响应体
...
<a id="redirect-post-request-with-payload-to-external-endpoint"></a>
##### 将带有负载的 POST 请求重定向到外部端点
...
<a id="route-to-different-backends-based-on-http-method"></a>
##### 基于 HTTP 方法路由到不同的后端
...
<a id="allow-multiple-cross-domains-using-the-cors-headers"></a>
##### 使用 CORS 头部允许多个跨域
...
<a id="set-correct-scheme-passed-in-x-forwarded-proto"></a>
##### 设置 X-Forwarded-Proto 中传递的正确协议
...
<a id="other-snippets"></a>
#### 其他片段

<a id="recreate-base-directory"></a>
###### 重建基础目录
...
<a id="create-a-temporary-static-backend"></a>
###### 创建临时静态后端
...
<a id="create-a-temporary-static-backend-with-ssl-support"></a>
###### 创建支持 SSL 的临时静态后端
...
<a id="generate-password-file-with-htpasswd-command"></a>
...
<a id="generate-private-key-without-passphrase"></a>
###### 生成无密码的私钥
...
<a id="generate-private-key-with-passphrase"></a>
###### 生成有密码的私钥
...
<a id="remove-passphrase-from-private-key"></a>
###### 移除私钥密码
...
<a id="encrypt-existing-private-key-with-a-passphrase"></a>
###### 使用密码加密现有私钥
...
<a id="generate-private-key-and-csr"></a>
###### 生成私钥和 CSR
...
<a id="generate-csr-metadata-from-existing-certificate"></a>
###### 生成 CSR（从现有证书获取元数据）
...
<a id="generate-csr-with--config-param"></a>
###### 使用 -config 参数生成 CSR
...
<a id="list-available-ec-curves"></a>
###### 列出可用 EC 曲线
...
<a id="print-ecdsa-private-and-public-keys"></a>
###### 打印 ECDSA 私钥和公钥
...
<a id="generate-ecdsa-private-key"></a>
###### 生成 ECDSA 私钥
...
<a id="generate-private-key-and-csr-ecc"></a>
###### 生成私钥和 CSR（ECC）
...
<a id="generate-self-signed-certificate"></a>
###### 生成自签名证书
...
<a id="generate-self-signed-certificate-from-existing-private-key"></a>
###### 从现有私钥生成自签名证书
...
<a id="generate-self-signed-certificate-from-existing-private-key-and-csr"></a>
###### 从现有私钥和 CSR 生成自签名证书
...
<a id="generate-multidomain-certificate-certbot"></a>
###### 生成多域名证书（Certbot）
...
<a id="generate-wildcard-certificate-certbot"></a>
###### 生成通配符证书（Certbot）
...
<a id="generate-certificate-with-4096-bit-private-key-certbot"></a>
###### 生成 4096 位私钥的证书（Certbot）

```bash
certbot certonly -d example.com -d www.example.com --rsa-key-size 4096
```

<a id="generate-dh-public-parameters"></a>
###### 生成 DH 公开参数
...
<a id="display-dh-public-parameters"></a>
###### 显示 DH 公开参数
...
<a id="extract-private-key-from-pfx"></a>
###### 从 pfx 提取私钥
...
<a id="extract-private-key-and-certs-from-pfx"></a>
###### 从 pfx 提取私钥和证书
...
<a id="verification-of-the-certificates-supported-purposes"></a>
###### 验证证书支持的用途
...
<a id="verification-of-the-private-key"></a>
###### 验证私钥
...
<a id="get-public-key-from-private-key"></a>
###### 从私钥获取公钥
...
<a id="verification-of-the-public-key"></a>
###### 验证公钥
...
<a id="verification-of-the-certificate"></a>
###### 验证证书
...
<a id="verification-of-the-csr"></a>
###### 验证 CSR
...
<a id="check-the-private-key-and-the-certificate-are-match"></a>
###### 检查私钥和证书是否匹配
...
<a id="check-the-private-key-and-the-csr-are-match"></a>
###### 检查私钥和 CSR 是否匹配
